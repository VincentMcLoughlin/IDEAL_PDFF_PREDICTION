...
class DataManager:
    ...
    def _make_train_test_split(self):
        ...
        val_fraction_of_test = self.validation_fraction / (1 - self.train_test_frac)
    ...
    def _build_dataset(self, data_info_df, batch_size):
        dataset = tf.data.Dataset.from_tensor_slices((data_info_df[self.data_column_name],data_info_df[self.score_column_name]))
    ...
    def build_datasets(self, batch_size):
        ...
        train_dataset = self._build_dataset(self.train_df, batch_size)
        test_dataset = self._build_dataset(self.test_df, batch_size)
